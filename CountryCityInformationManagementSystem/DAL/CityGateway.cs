...
namespace CountryCityInformationManagementSystem.DAL
{
    public class CityGateway:DBGateway
    {
        ...
        public List<CityViewerModel> GetCityInformation()
        {
            try
            {
                string query = "SELECT * FROM GetCountryInformation";
                CommandObj.CommandText = query;
                ConnectionObj.Open();
                ...
                while(reader.Read())
                {
                    ...
                    cityViewerModel.NoOfDewellers = Convert.ToInt32(reader["No_of_Dwellers"].ToString());
        ...
        public int GetTotalCityByCountryName(string name)
        {
            try
            {
                int total = 0;
                string query = "SELECT SUM(No_of_Dwellers) as Dwellers From [GetCountryInformation] WHERE Country_Name LIKE '%" + name + "%'";
                CommandObj.CommandText = query;
                ConnectionObj.Open();
                SqlDataReader reader = CommandObj.ExecuteReader();
                if (reader.Read())
                {
                  total = Convert.ToInt32(reader["Dwellers"].ToString());
                }
                reader.Close();
                ConnectionObj.Close();
                return total;
            ...
            {
                throw new Exception("Unable to collect Dwellers", exception);
            }
            finally
            {
                ConnectionObj.Close();
        ...
        public int GetTotalDwellersByCountryName(string name)
        {
            try
            {
                int totalCity = 0;
                string query = " SELECT count(*) as TotalCity From [GetCountryInformation] WHERE Country_Name LIKE '%" + name + "%'";
                CommandObj.CommandText = query;
                ConnectionObj.Open();
                SqlDataReader reader = CommandObj.ExecuteReader();
                if (reader.Read())
                {
                    totalCity = Convert.ToInt32(reader["TotalCity"].ToString());
                }
                reader.Close();
                ConnectionObj.Close();
                return totalCity;
            ...
            {
                throw new Exception("Unable to collect Citys", exception);
            }
            finally
            {
                ConnectionObj.Close();
